...
import { validateLevelAsc } from "genshin-optimizer/util";
...
function validateCharTCArtifactSlots(
  ...
  allArtifactSlotKeys.forEach((slotKey) => {
    (slots as ICharTC["artifact"]["slots"])[slotKey].level = clamp(
      (slots as ICharTC["artifact"]["slots"])[slotKey].level,
      0,
      artMaxLevel[(slots as ICharTC["artifact"]["slots"])[slotKey].rarity]
    );
  });
